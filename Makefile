...
SKILLS := arch-step miniarch-step arch-docs arch-mini-plan lilarch bugs-flow audit-loop comment-loop audit-loop-sim goal-loop north-star-investigation arch-flow arch-skills-guide agent-definition-auditor agents-md-authoring prompt-authoring chatgpt-web skill-authoring figma-best-practices fal-ai-tools flutter-reference eli10 pr-authoring commit-history-authoring skill-flow amir-publish codex-review-yolo fresh-consult agent-delegate plan-audit plan-implement plan-swarm agent-history model-consensus contact-sheet-builder exhaustive-code-review code-review stepwise arch-epic codex-cleanup thermo-nuclear-code-quality-review
