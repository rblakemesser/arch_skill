...
GEMINI_SKILLS := arch-step arch-step-goal-prompt miniarch-step arch-docs arch-mini-plan lilarch bugs-flow audit-loop comment-loop audit-loop-sim goal-loop north-star-investigation arch-flow arch-skills-guide agent-definition-auditor agents-md-authoring prompt-authoring browseros chatgpt-web skill-authoring figma-best-practices fal-ai-tools flutter-reference pr-authoring commit-history-authoring amir-publish codex-review-yolo fresh-consult agent-delegate plan-audit plan-implement plan-conductor model-consensus contact-sheet-builder fc-branded-pdf cf-share cynical-code-review cynical-architecture-review cynical-cruft-removal exhaustive-code-review stepwise arch-epic codex-cleanup thermo-nuclear-code-quality-review
