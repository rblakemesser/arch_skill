#!/usr/bin/env python3
"""Install or verify the audit-loop Stop hook in Codex hooks.json."""
...
STATUS_MESSAGE = "audit-loop automatic controller is running; fresh review passes can take a few minutes"
HOOK_SCRIPT_NAME = "audit_loop_stop_hook.py"
...
def expected_command(skills_dir: Path) -> str:
    hook_script = skills_dir / "audit-loop" / "scripts" / HOOK_SCRIPT_NAME
...
def is_audit_loop_group(group: object) -> bool:
...
def install_hook(hooks_file: Path, skills_dir: Path) -> None:
    ...
    stop_groups = [group for group in stop_groups if not is_audit_loop_group(group)]
...
def verify_hook(hooks_file: Path, skills_dir: Path) -> None:
    ...
    for group in stop_groups:
        if group == wanted:
            return
    raise SystemExit(
        "missing audit-loop Stop hook entry in "
