...
after each new clean `review` critic has returned and the parent has accepted
the verdict. The critic is a same-host native child by default while the parent
session is active: Codex uses `fork_turns: "none"`; Claude uses a clean named
or custom subagent rather than a bare conversation fork or skill
