...
`_audit_sim_ledger.md` after each new clean `review` critic has returned and
the parent has accepted the verdict. The critic is a same-host native child by
...
continuity; it does not create review independence or new simulator/device
authority. Never use external transport to bypass unavailable or unauthorized
sanctioned capabilities. The parent remains the owner of the ledger, result
