...
3. Is the ask for broad, ambiguity-heavy, or helper-heavy full arch work?
   - use `arch-step`
4. Is the ask for faster full-arch work on a smaller well-defined feature, still with phasing and native goal-mode auto flow?
   - use `miniarch-step`
...
11. Is it a small feature or improvement that should fit in 1-3 phases?
   - use `lilarch`
12. Does the user want a one-pass mini plan with canonical arch blocks?
   - use `arch-mini-plan`
13. Otherwise, default to `arch-step`.
...
- Native `/goal` vs specialized loops (`audit-loop`, `comment-loop`, `audit-loop-sim`):
  ...
  - use native `/goal` when requirements are free-form and no arch-skill artifact contract is needed
